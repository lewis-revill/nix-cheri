...
cfg_if::cfg_if! {
    ...
    pub fn handler(&self) -> SigHandler {
        ...
        if self.flags().contains(SaFlags::SA_SIGINFO) {
            return SigHandler::SigAction(
                unsafe {
                    *(self.sigaction.sa_u.sa_sigaction as *const extern fn(_, _, _))
                } as extern fn(_, _, _))
        }
        SigHandler::Handler(
            unsafe {
                *(self.sigaction.sa_u.sa_handler.sah_fn as *const extern fn(libc::c_int))
